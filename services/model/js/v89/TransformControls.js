	...
	module.exports = function(THREE) {
		...
		THREE.TransformGizmoTranslate = function(options) {
			...
			if (options && options.direction) {
				var tmpGizmos = {};
				tmpGizmos[options.direction] = this.handleGizmos[options.direction];
				this.handleGizmos = tmpGizmos;
			}
			...
			if (options && options.direction) {
				var tmpGizmos = {};
				tmpGizmos[options.direction] = this.pickerGizmos[options.direction];
				this.pickerGizmos = tmpGizmos;
			}
		...
		THREE.TransformControls = function(camera, domElement, options) {
			...
			if (options.translate) {
				this.translateMaxBounds = options.translate.maxBounds;
				this.translateMinBounds = options.translate.minBounds;
			}
			...
			var _dragging = false;
			var _gizmo = {
				"translate": new THREE.TransformGizmoTranslate(options.translate),
				"rotate": new THREE.TransformGizmoRotate(options.rotate),
				"scale": new THREE.TransformGizmoScale(options.scale)
			};
			...
			function onPointerMove(event) {
				...
				if (scope.enabled) {
					...
					if (_mode === "translate") {
						...
						if (scope.translateMaxBounds) {
							for (var direction in scope.translateMaxBounds) {
								if (scope.object.position[direction.toLowerCase()] < scope.translateMaxBounds[direction]) {
									scope.object.position[direction.toLowerCase()] = scope.translateMaxBounds[direction];
								}
							}
						}

						if (scope.translateMinBounds) {
							for (var direction in scope.translateMinBounds) {
								if (scope.object.position[direction.toLowerCase()] > scope.translateMinBounds[direction]) {
									scope.object.position[direction.toLowerCase()] = scope.translateMinBounds[direction];
								}
							}
						}
